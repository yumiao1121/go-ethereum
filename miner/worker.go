...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address) (*environment, error) {
	...
	env := &environment{
		...
		state:     state,
		coinbase:  coinbase,
		ancestors: mapset.NewSet[common.Hash](), // daewoo: 创建common.Hash类型的集合，只包含叔块
		family:    mapset.NewSet[common.Hash](), // daewoo: 包含祖先块和叔块
		header:    header,
	...
	for _, ancestor := range w.chain.GetBlocksFromHash(parent.Hash(), 7) {
		for _, uncle := range ancestor.Uncles() {
			env.family.Add(uncle.Hash()) // daewoo: 填充高度7之内的祖先块和包含的叔块
		}
...
type generateParams struct {
	...
	withdrawals types.Withdrawals // List of withdrawals to include in block. daewoo: 表示共识层验证者的提款
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if genParams.parentHash != (common.Hash{}) { // daewoo: 表示不是生成最新块
	...
	if parent.Time >= timestamp {
		if genParams.forceTime { // daewoo: 必须使用给定的timestamp
